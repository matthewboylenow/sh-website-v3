...
import { leadsByUser } from "./_actions";
...
export default async function AdminUsersPage() {
  ...
  const leadMap = await leadsByUser(rows.map((r) => r.id));
      ...
      <header className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="sh-lede mt-2 text-[15px]">
            Invite editors and ministry leads. A single user can lead any
            number of ministries; a ministry can have any number of leads.
          </p>
        </div>
      </header>
        ...
        <table className="w-full text-left text-sm">
          <thead>
            <tr className="border-b border-rule bg-cream text-[11px] uppercase tracking-[0.12em] text-ink-3">
              ...
              <th className="px-4 py-3 font-semibold">Role · Ministries</th>
            </tr>
          </thead>
          <tbody>
            {rows.map((r) => {
              const myMinistryIds = leadMap.get(r.id) ?? [];
              return (
                <tr key={r.id} className="border-b border-rule last:border-0 align-top">
                  ...
                  <td className="px-4 py-3">
                    <RoleSelector
                      userId={r.id}
                      currentRole={r.role}
                      currentMinistryIds={myMinistryIds}
